...
- **Reorder на Today** — тот же grip-handle; порядок в `actions.today_sort_order`, не влияет на `sort_order` в документе.
...
- Показывает actions с `today = true`, порядок — `today_sort_order` (drag-and-drop на `/today`).
